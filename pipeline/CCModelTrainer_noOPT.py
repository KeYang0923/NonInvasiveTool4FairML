# Learn the CC rules from training data
...
from utils import format_print, save_json

warnings.filterwarnings('ignore')

def return_identify_zero_violation(dense_df, sensi_col, y_col, group_names, label_names, cc_cols,
                                 n_groups=2, n_labels=2, verbose=True,
                                save_print_f=None):
    ...
    for group_i, group_name in zip(range(n_groups), group_names):
        for label_i, label_name in zip(range(n_labels), label_names):
            dense_col_name = '_'.join(['noOPT', 'G' + str(group_i), 'L' + str(label_i)])
            #
            ...
            # di_train_dense = di_train.iloc[:dense_n, :].copy() # dense_n points with highest density score

            assertions_i = di.learn_assertions(di_train[cc_cols], max_self_violation=1.0)
            ...
            result_train = assertions_i.evaluate(di_train[cc_cols], explanation=True, normalizeViolation=True)
...
def learn_cc_models(data_name, seed, y_col, sensi_col, name_mapping, cc_skiped_cols, special_suffix=None,
                    res_path='../intermediate/models/',
                    n_groups=2, n_labels=2,
                    data_path='../data/processed/'):

    cur_dir = res_path + data_name + '/'
    cc_print_file = cur_dir + '-'.join(['train_CC', str(seed), special_suffix])

    group_names, label_names, dense_kernal = name_mapping

    input_df = pd.read_csv(data_path + data_name + '_dense.csv')

    train_df, validate_df, test_df = split(input_df, seed) # TODO: update validation set for the parameter selection of CC models for density kernal, top_n in density selection
    ...
    cc_models, core_conform_index, opp_conform_index = return_identify_zero_violation(train_df, sensi_col, y_col, group_names, label_names,
                                               cc_cols, save_print_f=cc_print_file)
    ...
    test_df.to_csv(cur_dir+'-'.join(['test_violation', str(seed), special_suffix])+'.csv', index=False)


    # dense_train.to_csv(cur_dir + '-'.join(['train_density', str(seed)]) + '.csv', index=False)
    # save sample index of train from the four cases
    save_json(weights, cur_dir+'-'.join(['train_weights', str(seed), special_suffix]))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--setting", type=str, default='noOPT',
    ...
    if args.run == 'parallel':
        ...
        for data_name, y_col, sensi_col in zip(datasets, y_cols, sensi_cols):
            ...
            for seed in seeds:
                tasks.append([data_name, seed, y_col, sensi_col, name_mapping, cc_skip_cols, args.setting, res_path])
