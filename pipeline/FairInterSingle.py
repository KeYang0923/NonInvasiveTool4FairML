...
def retrain_models_by_faircc(data_name, seed, y_col, sensi_col, fair_setting, weights_input=None, sensi_col_in_training=1, output_path='../intermediate/models/',
                             special_suffix=None,
    ...
    train_df = pd.read_csv(out_dir + '-'.join(['train', str(seed), set_suffix]) + '.csv', index_col=train_index_col)
    validate_df = pd.read_csv(out_dir + '-'.join(['val', str(seed), set_suffix]) + '.csv')
    test_df = pd.read_csv(out_dir + '-'.join(['test', str(seed), set_suffix]) + '.csv')
    ...
    if special_suffix is not None:
        dump(model, out_dir + '-'.join(['model', str(seed), fair_setting, set_suffix, special_suffix]) + '.joblib')
        validate_df[['Y', 'Y_pred_scores']].to_csv(out_dir + '-'.join(['y_val', str(seed), fair_setting, set_suffix, special_suffix]) + '.csv',
                                                   index=False)
        train_df.to_csv(out_dir + '-'.join(['train_weights', str(seed), fair_setting, set_suffix, special_suffix]) + '.csv', index=False)
    else:
        dump(model, out_dir + '-'.join(['model', str(seed), fair_setting, set_suffix]) + '.joblib')
        validate_df[['Y', 'Y_pred_scores']].to_csv(out_dir + '-'.join(['y_val', str(seed), fair_setting, set_suffix]) + '.csv', index=False)
        train_df.to_csv(out_dir + '-'.join(['train_weights', str(seed), fair_setting, set_suffix]) + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    if args.run == 'parallel':
        ...
        for data_name, y_col, sensi_col in zip(datasets, y_cols, sensi_cols):
            for seed in seeds:
                tasks.append([data_name, seed, y_col, sensi_col, args.setting, weights_adjust, args.sensi, res_path])
