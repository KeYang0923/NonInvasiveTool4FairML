...
def apply_models_by_cc(data_name, seed, y_col, sensi_col, sensi_col_in_training, res_path='../intermediate/models/', error_suffix=None,
    ...
    else: # original data, using the sensi_inclusion version because of the same version of test set is stored in disc no matter the value of sensi_col_in_training
    ...
    for group_suffix in [None, 'G0', 'G1']:
        model = load_model(cur_dir, seed, set_suffix, group_suffix)
        if group_suffix is not None:
            thres_file = cur_dir + '-'.join(['Thres', str(seed), group_suffix, set_suffix])
            pred_col = 'Y_pred_'+group_suffix
        else:
            thres_file = cur_dir + '-'.join(['Thres', str(seed), set_suffix])
            pred_col = 'Y_pred'
    ...
    if 'error' in error_suffix: # for erroneous records
        test_vio[[sensi_col, 'Y', 'Y_pred', 'Y_pred_G0', 'Y_pred_G1', 'Y_pred_en', 'Y_pred_en_cc']].to_csv(cur_dir + '-'.join(['test_eval_multi', str(seed), error_suffix, set_suffix]) + '.csv', index=False)
    else:
        test_vio[[sensi_col, 'Y', 'Y_pred', 'Y_pred_G0', 'Y_pred_G1', 'Y_pred_en', 'Y_pred_en_cc']].to_csv(cur_dir + '-'.join(['test_eval_multi', str(seed), set_suffix]) + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    #     raise ValueError('The input "setting" is requried. Use "drift" for original test without errors OR "error"+0.10 for the example of 10% errors simulated in the test set.')
    #
    ...
    #
    # else: # erroneous case
    #     if args.setting not in ['error0.30', 'error0.20', 'error0.25', 'error0.15', ' error0.10', 'error0.05', 'error0.01']:
    #         raise ValueError(
    #             'The input "setting" is not supported. Use "drift" for original test without errors OR "error"+0.10 for the example of 10% errors simulated in the test set. OR run "ErrorDataSimulator.py" for new desired rates.')
    ...
    if args.run == 'parallel':
        ...
        for data_name, y_col, sensi_col in zip(datasets, y_cols, sensi_cols):
            for seed in seeds:
                tasks.append([data_name, seed, y_col, sensi_col, args.sensi, res_path, cc_setting])
