...
def LR_trainer(data_name, y_col, sensi_col, seed, res_path='../intermediate/models/',
    ...
    dump(model, cur_dir + '-'.join(['model', str(seed)]) + '.joblib')
    validate_df[['Y', 'Y_pred_scores']].to_csv(cur_dir + '-'.join(['y_val', str(seed)]) + '.csv', index=False)

    train_df.to_csv(cur_dir + '-'.join(['train', str(seed)]) + '.csv') # keep index for sanity check of random splits
    validate_df.to_csv(cur_dir + '-'.join(['val', str(seed)]) + '.csv', index=False)
    test_df.to_csv(cur_dir + '-'.join(['test', str(seed)]) + '.csv', index=False)
    ...
    for group_i in range(n_groups):
        ...
        dump(group_model, cur_dir + '-'.join(['model', str(seed), 'G' + str(group_i)]) + '.joblib')
        group_val_df[['Y', 'Y_pred_scores']].to_csv(cur_dir + '-'.join(['y_val', str(seed), 'G' + str(group_i)]) + '.csv', index=False)
...
if __name__ == '__main__':
    ...
    if args.run == 'parallel':
        ...
        for data_name, y_col, sensi_col in zip(datasets, y_cols, sensi_cols):
            for seed in seeds:
                tasks.append([data_name, y_col, sensi_col, seed, res_path])
