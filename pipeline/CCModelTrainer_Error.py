...
def learn_cc_models(data_name, seed, y_col, sensi_col, name_mapping, cc_skiped_cols, dense_skiped_cols,
                    res_path='../intermediate/models/', error_suffix=None,
    ...
    cc_print_file = cur_dir + '-'.join(['train_CC', str(seed), error_suffix])
    ...
    train_df = pd.read_csv(cur_dir + '-'.join(['train', str(seed), error_suffix]) + '.csv')
    test_df = pd.read_csv(cur_dir + '-'.join(['test', str(seed)]) + '.csv')

    dense_cols = sorted(list(set(train_df.columns) - set(dense_skiped_cols+[sensi_col, y_col, 'Unnamed: 0'])))

    # estimate desnsity
    dense_train = estimate_density(train_df, sensi_col, y_col, dense_cols, dense_kernal=dense_kernal)

    cc_cols = sorted(list(set(train_df.columns) - set(cc_skiped_cols+[sensi_col, y_col, 'Unnamed: 0'])))
    ...
    test_df.to_csv(cur_dir+'-'.join(['test_violation', str(seed), error_suffix])+'.csv', index=False)

    dense_train.to_csv(cur_dir + '-'.join(['train_density', str(seed), error_suffix]) + '.csv', index=False)
    # save sample index of train from the four cases
    save_json(weights, cur_dir+'-'.join(['train_weights', str(seed), error_suffix]))
...
if __name__ == '__main__':
    ...
    if args.run == 'parallel':
        ...
        for data_name, y_col, sensi_col in zip(datasets, y_cols, sensi_cols):
            ...
            for seed in seeds:
                tasks.append([data_name, seed, y_col, sensi_col, name_mapping, cc_skip_cols, dense_skip_cols, res_path, args.error])
