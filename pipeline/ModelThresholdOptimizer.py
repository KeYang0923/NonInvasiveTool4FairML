...
def load_model(dir, seed, sensi_suffix, group_suffix=None, return_val_data=True):
    if group_suffix is not None:
        model = load(dir + '-'.join(['model', str(seed), group_suffix, sensi_suffix]) + '.joblib')
        Y_val_df = pd.read_csv(dir + '-'.join(['y_val', str(seed), group_suffix, sensi_suffix]) + '.csv')
    else:
        model = load(dir + '-'.join(['model', str(seed), sensi_suffix]) + '.joblib')
        Y_val_df = pd.read_csv(dir + '-'.join(['y_val', str(seed), sensi_suffix]) + '.csv')
...
def compute_model_optimal_thresh(data_name, seed, group_suffix, sensi_col_in_training, path, opt_obj, special_suffix=None):
    cur_dir = path + data_name + '/'
    if special_suffix is not None: # TODO: remove after verifying it is not being used
        group_suffix = group_suffix + '-' + special_suffix

    set_suffix = 'S_{}'.format(sensi_col_in_training)

    model, Y_val_df = load_model(cur_dir, seed, set_suffix, group_suffix=group_suffix)
    opt_thres = find_optimal_thres(Y_val_df, opt_obj=opt_obj)

    if group_suffix is not None:
        save_json(opt_thres, cur_dir+'-'.join(['Thres', str(seed), group_suffix, set_suffix]))
    else:
        save_json(opt_thres, cur_dir + '-'.join(['Thres', str(seed), set_suffix]))
        ...
        for data_name in datasets:
            for seed in seeds:
                for group_suffix in settings:
                    tasks.append([data_name, seed, group_suffix, args.sensi, res_path, opt_obj])
