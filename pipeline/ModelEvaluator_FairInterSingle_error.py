...
def eval_single_models(data_name, seed, y_col, sensi_col, fair_setting, error_suffix, res_path='../intermediate/models/'):
    ...
    cur_model = load_model_lr(cur_dir, seed, fair_setting+'-'+error_suffix)
    cur_thres = read_json(cur_dir + '-'.join(['Thres', str(seed), fair_setting, error_suffix]))['thres']
    ...
    test_df[[sensi_col, 'Y', 'Y_pred_cc']].to_csv(cur_dir + '-'.join(['test_eval_single', str(seed), fair_setting, error_suffix]) + '.csv', index=False)

    setting_output = {}
    for pred_col, setting in zip(['Y_pred_cc'], ['C']):
        setting_output[setting] = eval_settings(test_df, sensi_col, pred_col)
    save_json(setting_output, cur_dir + '-'.join(['Single_eval', str(seed), fair_setting, error_suffix]))
...
if __name__ == '__main__':
    ...
    if args.run == 'parallel':
        ...
        for data_name, y_col, sensi_col in zip(datasets, y_cols, sensi_cols):
            for seed in seeds:
                tasks.append([data_name, seed, y_col, sensi_col, args.setting, args.error, res_path])
