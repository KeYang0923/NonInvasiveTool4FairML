...
def compute_model_optimal_thresh(data_name, seed, group_suffix, path, opt_obj, special_suffix=None):
    ...
    if group_suffix not in [None, 'G0', 'G1']:
        if special_suffix is not None: # for singleCC + noOPT
    ...
    if group_suffix is not None:
        if special_suffix is not None:
            save_json(opt_thres, cur_dir+'-'.join(['Thres', str(seed), group_suffix, special_suffix]))
        else:
        ...
        if special_suffix is not None:
            ...
            save_json(opt_thres, cur_dir + '-'.join(['Thres', str(seed), special_suffix]))
        ...
        for data_name in datasets:
            for seed in seeds:
                for group_suffix in settings:
                    tasks.append([data_name, seed, group_suffix, res_path, opt_obj, args.special])
