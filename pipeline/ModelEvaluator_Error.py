...
if __name__ == '__main__':
    ...
    if args.setting is None:
        raise ValueError('The input "setting" is requried. Use "all" for multiple error rates. OR "error"+0.10 for the example of 10% errors simulated in the test set.')
        ...
        if args.setting != 'all':
            raise ValueError(
                'The input "setting" is not supported.  Use "all" for multiple error rates. OR "error"+0.10 for the example of 10% errors simulated in the test set.')
        else:
        ...
        if args.setting not in ['error0.30', 'error0.25', 'error0.20', 'error0.15', 'error0.10', 'error0.05', 'error0.01']:
            raise ValueError(
                'The input "setting" is not supported. Use "all" for multiple error rates. OR choose from [error0.10, error0.15, error0.20, error0.30, error0.01, error0.05]. OR run "ErrorDataSimulator.py" for new desired rates.')
        else:
