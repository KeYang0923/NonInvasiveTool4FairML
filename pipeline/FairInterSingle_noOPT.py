...
def retrain_models_by_faircc(data_name, seed, y_col, sensi_col, fair_setting, weights_input=None,
    ...
    if special_suffix is not None:
        cc_weights = read_json(out_dir + '-'.join(['train_weights', str(seed), special_suffix]))
    else:
...
if __name__ == '__main__':
    ...
    if args.run == 'parallel':
        ...
        for data_name, y_col, sensi_col in zip(datasets, y_cols, sensi_cols):
            for seed in seeds:
                tasks.append([data_name, seed, y_col, sensi_col, args.setting, weights_adjust, res_path, args.special])
