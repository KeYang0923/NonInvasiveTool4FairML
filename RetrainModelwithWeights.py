...
def retrain_LR_weights(data_name, seed, reweigh_method, weight_base, input_degree=None, res_path='../intermediate/models/',
    ...
    if reweigh_method == 'omn':
        save_json({'time': time, 'find': best_find}, '{}mltime-{}-{}-{}.json'.format(cur_dir, seed, reweigh_method, weight_base))
    else:
        save_json({'time': time}, '{}mltime-{}-{}-{}.json'.format(cur_dir, seed, reweigh_method, weight_base))
...
if __name__ == '__main__':
    ...
    if args.run == 'parallel':
        ...
        with Pool(cpu_count()//2) as pool:
            pool.starmap(retrain_LR_weights, tasks)
    else:
